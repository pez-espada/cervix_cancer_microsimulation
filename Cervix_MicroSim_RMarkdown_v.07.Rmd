...
Costs_per_Cancer_Diag <- function (M_it, cost_Vec, symptomatics, time_iteration, Trt = FALSE) {
  ## Debugging 
  #symptomatics %>% print()
  #symptomatics %>% nrow() %>% print()
  #symptomatics %>% nrow() %>% class() %>% print()
  #
  #if(nrow(symptomatics) > 0 | !is.null(nrow(symptomatics))) {print("Yeah nrow is larger than 0!")}
...
MicroSim <- function(v_M_1, n_i, n_t, v_n, d_c, d_e, TR_out = TRUE, 
  ...
  for (t in 1:(n_t)) {
    ...
    m_C[, t + 1] <-                              # estimate costs per individual
      Costs_per_Cancer_Diag(M_it = m_M[, t + 1],              # during cycle t + 1
