...
################################################################################
## ─── ARGUMENT PARSING FOR SLURM ARRAY ───────────────────────────────────
# Grab everything, then drop "--args" if it snuck in
raw_args <- commandArgs(trailingOnly = TRUE)
args <- raw_args[ raw_args != "--args" ]

cat(">> Raw args:   ", paste(raw_args, collapse = " | "), "\n")
cat(">> Clean args: ", paste(args,     collapse = " | "), "\n")

# Now enforce exactly three
if (length(args) != 3) {
  stop("ERROR: expected exactly 3 coverage values but got: ", length(args))
}
...
vacc_coverage <- as.numeric(args)
if (any(is.na(vacc_coverage))) {
  stop("ERROR: non‐numeric coverage value(s) in: ", paste(args, collapse = ", "))
}
