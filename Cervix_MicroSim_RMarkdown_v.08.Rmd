...
################################################################################
# version 3 (previous versions in the older script versions)
...
# Cost and utility inputs (to be modified..)
...
Costs_per_Cancer_Diag <- function (M_it, cost_Vec, symptomatics, time_iteration, Trt = FALSE) {
  ...
  if(nrow(symptomatics) > 0 ) {
    ## Debugging:
    #print("Yeah, nrow is larger than 0!")
    #paste("The time step is", time_iteration) %>% print()
...
# version 3 (previous version not displayed in this script version)
...
MicroSim <- function(strategy="natural_history", numb_of_sims = 1, 
  ...
  for(sim in 1:numb_of_sims){
    ...
    for (t in 1:(n_t)) {
      ...
      ## Debugging: 
      ## Ensure M_it has no NA values
      #m_M[is.na(m_M)] <- "H"  # Or any other default state that makes sense
      ############################################################################ 
