...
# Concatenate the prevalence to the sim result 
CC_mortality_result <-
  CC_mortality_func(sim_stalked_result = incidence_result, my_Probs = my_Probs)  
...
other_mortality_func <- function(sim_stalked_result, my_Probs) {
  ...
  df <- sim_stalked_result[[1]]$TR %>% 
    ...
    dplyr::mutate(age_interval = cut(age, breaks = breaks, labels = labels, right = FALSE)) %>% 
    dplyr::group_by(age_interval) %>% 
    dplyr::summarise(mean_other_mortality = mean(other_mortality, na.rm = TRUE)) %>% 
    dplyr::ungroup()
  
  #return(df)
  sim_stalked_result[[1]]$mean_other_mortality <- df
  return(sim_stalked_result)
}
################################################################################

# Concatenate the prevalence to the sim result 
other_mortality_result <-
  other_mortality_func(sim_stalked_result = CC_mortality_result, my_Probs = my_Probs)  
