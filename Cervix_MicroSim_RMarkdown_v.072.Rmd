...
incidence_func <- function(sim_stalked_result, state, my_Probs) { # work in progress
  ...
  # sum_of_previous_states_at_tminus1 <- 

  # Defining dfs:
  #TR <- sim_stalked_result[[1]]["TR"]
  #TR <- sim_stalked_result[[1]]$TR
  ...
  my_incidence_df <- my_incidence_df %>%
    mutate(new_col = get(target_column) / lag(rowSums(select(., all_of(all_previous_states))), default = NA))
  
  
  ### Use the ensym() function to convert strings to symbols
  #library(rlang)
  #new_state <- ensym(new_state)
  #
  ## Defining new_cases state:
  #new_state    <- paste0("new_", state)
  #new_state_df <- sim_stalked_result[[1]][new_state]
  ...
  df <- sim_stalked_result[[1]]$TR %>% 
    ...
    deplyr::mutate(total_alive = H + CIN1 + CIN2 +CIN3 +
                     FIGO.I + FIGO.II + FIGO.III + FIGO.IV + Survival) %>%
    #mutate(prevalence = HR.HPV.infection / H) %>% 
    deplyr::mutate(prevalence = HR.HPV.infection / total_alive) %>% 
    deplyr::mutate(age_interval = cut(age, breaks = breaks, labels = labels, right = FALSE)) %>% 
    group_by(age_interval) %>% 
    summarise(prevalence = mean(prevalence, na.rm = TRUE)) %>% 
    ungroup()
