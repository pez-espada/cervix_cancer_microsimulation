...
prevalence_func <- function(sim_stalked_result, my_Probs) {
  ...
  df <- sim_stalked_result[[1]]$TR %>% 
    ...
    dplyr::mutate(total_alive = H + CIN1 + CIN2 + CIN3 +
...
incidence_result <- list()

for (my_state in incidence_states_to_compute) {
  print(my_state)
  incidence_result <- incidence_func(sim_stalked_result = prevalence_result, state = my_state, my_Probs = my_Probs)  
}
