...
mean_CC_mortality_result <-
  CC_mean_mortality_func(sim_stalked_result = mean_CC_mortality_result, my_Probs = my_Probs)  
...
markov_CC_mortality <- c(0.000000e+00, 0.000000e+00, 0.000000e+00, 2.977975e-06, 1.574920e-05, 2.715056e-05, 5.489929e-05, 7.284815e-05, 1.057494e-04, 5.076268e-05, 7.517773e-05, 4.960943e-05, 4.802468e-05, 4.210457e-05, 4.837655e-05) 
...
```{r, ploting incidences and prevalences, echo=FALSE}
# Check the type of the objects
str(microSim_CN1_incidences)
str(microSim_CN2_incidences)
# Repeat for other vectors

# Convert to numeric if necessary
markov_data <- data.frame(
    age = as.numeric(1:15),
    CN1_incidences = as.numeric(markov_CN1_incidences),
    CN2_incidences = as.numeric(markov_CN2_incidences),
    CN3_incidences = as.numeric(markov_CN3_incidences),
    CC_incidences = as.numeric(markov_CC_incidences),
    HPV_prevalences = as.numeric(markov_HPV_prevalences),
    CC_mortality = as.numeric(markov_CC_mortality)
)

microSim_data <- data.frame(
    age = as.numeric(1:15),
    CN1_incidences = as.numeric(microSim_CN1_incidences),
    CN2_incidences = as.numeric(microSim_CN2_incidences),
    CN3_incidences = as.numeric(microSim_CN3_incidences),
    CC_incidences = as.numeric(microSim_CC_incidences),
    HPV_prevalences = as.numeric(microSim_HPV_prevalences),
    CC_mortality = as.numeric(microSim_CC_mortality)
)

ggplot(combined_data, aes(x = factor(age), y = value, fill = model)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_wrap(~metric, scales = "free_y") +
  theme_minimal() +
  labs(x = "Age", y = "Value", title = "Comparison of Markov and Microsimulation Models") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Markov" = "skyblue", "Microsimulation" = "salmon"))
