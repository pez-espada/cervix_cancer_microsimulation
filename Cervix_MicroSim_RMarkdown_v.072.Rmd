...
CC_mean_CC_incidence_func <- function(sim_stalked_result, my_Probs) {
  # Extract unique age intervals
  age_intervals <- my_Probs %>% 
    select(Lower, Larger) %>% 
    unique() %>% 
  ...
  # Compute prevalence and average it by age intervals
  df <- sim_stalked_result[[1]]$ %>% 
    #dplyr::select(sim, cycle, age, H, HR.HPV.infection) %>% 
    dplyr::select(everything()) %>% 
    dplyr::mutate(total_alive = H + HR.HPV.infection + CIN1 + CIN2 + CIN3 +
                    FIGO.I + FIGO.II + FIGO.III + FIGO.IV + Survival) %>%
    dplyr::mutate(CC_mean_incidence = (CC_Death / total_alive) * 10^5) %>% 
    dplyr::mutate(age_interval = cut(age, breaks = breaks, labels = labels, right = FALSE)) %>% 
    dplyr::group_by(age_interval) %>% 
...
microSim_CC_incidences   <- other_mean_mortality_result[[1]]$
microSim_HPV_prevalences <- other_mean_mortality_result[[1]]$mean_HPV_prevalence_per_age_interval
markov_CC_mortality      <- other_mean_mortality_result[[1]]$CC_mean_mortality
...
<!-- df_CIN1_nc    <- other_mean_mortality_result[[1]]$new_CIN1 -->
<!-- df_CIN2_nc    <- other_mean_mortality_result[[1]]$new_CIN2 -->
<!-- df_CIN3_nc    <- other_mean_mortality_result[[1]]$new_CIN3 -->
<!-- df_Cancer_nc  <- other_mean_mortality_result[[1]]$new_Cancer -->
...
<!--   #                `new cases FIGO3` + -->
<!--   #                `new cases FIGO4`) -->
<!-- rm(df_CIN1_nc, df_CIN2_nc, df_CIN3_nc, df_Cancer_nc)  -->
<!--    #df_FIGO1_nc, df_FIGO2_nc,  -->
<!--    #df_FIGO3_nc, df_FIGO4_nc) -->
