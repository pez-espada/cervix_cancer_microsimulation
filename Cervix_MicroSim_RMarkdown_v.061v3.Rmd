...
n_t   <- 34                   # time horizon, 75 cycles (it starts from 0)
...
#my_symptomatics <- data_frame()
my_symptomatics <- tibble()
### Costs function
...
Costs <- function (M_it, cost_Vec, diagnosed_list, Trt = FALSE) {
  ...
  if (length(diagnosed_list) > 0) {
    print("yes!")
    paste0("length or row.names of diagnose_list is: ", row.names(diagnosed_list)) %>% print()
    my_symptomatics <- bind_rows(diagnosed_list)
  ...
  #my_symptomatics <- bind_rows(diagnosed_list)
  # Debugging:
  my_symptomatics %>% print()
  #print(my_symptomatics)
  #print("###########################")
  #print("###########################")
...
# This function takes a column `col` of dimensions `n_i x 1` and a `time_step`
# corresponding to the current cycle. It returns a list where the first element
# is a data frame containing the NEW cases diagnosed with cancer and an updated
# column for those cases that have been cured and moved to the 'survival' state.
...
MicroSim <- function(v_M_1, n_i, n_t, v_n, d_c, d_e, TR_out = TRUE, 
  ...
  for (t in 1:(n_t)) {
    ############################################################################
    # 0. Select the transition matrix based on the cycle `n_t`:
    # Since our age intervals start at 10 years old,
    # `age_in_loop` is calculated as `t + 10 * age_factor(cycle_period)`.
    age_in_loop <- t + 10
    # 0.1. Choose corresponding transition matrix according current age:
    ...
    ## 0.2 Check what's the cycle period and modify transitions accordingly:
    #my_age_prob_matrix <- 
    ...
    ############################################################################    
    # 1. Extract the transition probabilities of each individuals at cycle t
    ...
    ############################################################################    
    # 2. Sampling - make actual transitions
    ...
    ############################################################################ 
    # 3.Check diagnose and determine survivals:
    ...
    # `results` stores NEW diagnosed cancer cases: 
