...
# For the last cylce/iteration we need to adjust the last transition matrix:
my_Probs$Larger <- 
...
Probs <- function(M_it, my_Probs) {
  ...
  for (i in 1:length(v_n)) {
    ...
    if (sum(state_mask) > 0) {
      m_P_it[, state_mask] <- lapply(X = v_n, function(x)
        trans_prb(P = my_Probs, state1 = v_n[i], state2 = x)) %>%
        unlist()
    } else {
    }
  }
...
### This function is not used -so we can just dispose of it? #####
...
# Function receives a column with the new entries computed by the 'diagnose_column`
# function and gives back the updated_next column
#################################################################################
...
# This version produces a unique master data frame with the solutions of all
# simulations stacked
MicroSim <- function(strategy="natural_history", numb_of_sims = 1, 
                     v_M_1, n_i, n_t, v_n, d_c, d_e, TR_out = TRUE, 
                     TS_out = TRUE, Trt = FALSE, sim_numb = 1, seed = 1, Pmatrix) {
  ## Debuging:
  #TR_out = TRUE; TS_out = TRUE; Trt = FALSE; seed = 1
  
  #Arguments:
  # strategy:     strategy name, "natural_history" by default,
  #numb_of sims:  number of simulations, 1 by default
  ...
  for(sim in 1:numb_of_sims){
    ...
    for (t in 1:(n_t)) {
      ...
      m_C[, t ] <-                              # estimate costs per individual
        Costs_per_Cancer_Diag(M_it = m_M[, t],  # during cycle t + 1
                              symptomatics = symptomatics,
                              time_iteration = t,
      ...
      m_E[, t + 1] <-                              # estimate QALYs per individual
        Effs( m_M[, t + 1], Trt, 
              utilityCoefs = utilityCoefs)                   # during cycle t + 1
      ############################################################################    
    ...
    all_results[[sim]] <- results  # Store results in the list
    
    # Clear large matrices from memory if not needed anymore
    #rm(m_M, m_C, m_E)
    
  }  # end of the MicroSim function  
  
  # Convert each element to a data frame if it is not already
...
#sim_no_trt  <- MicroSim(v_M_1, n_i, n_t, v_n, d_c=d_c, d_e, Trt = FALSE)
sim_no_trt  <- MicroSim(strategy = "natural_history",numb_of_sims = 40, 
                        v_M_1 = v_M_1, n_i = n_i, n_t = n_t, v_n = v_n, 
                        d_c = d_c, d_e = d_e, TR_out = TRUE, TS_out = TRUE, 
                        Trt = FALSE, sim_numb = 1, seed = 1, Pmatrix = Pmatrix)
## run for treatment
#sim_trt     <- MicroSim(v_M_1, n_i, n_t, v_n, d_c, d_e, Trt = TRUE)  
comp.time = Sys.time() - p
comp.time %>% print()
saveRDS(object = sim_no_trt, file = "microsim_ver.8_75x40x10E6.rds")
